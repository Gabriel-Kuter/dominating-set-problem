...
#pragma omp cancel for
        ...
        // Printa o resultado de cada replicação
        std::ofstream rawFile("results_raw.txt", std::ios::app);

        // nome da instancia
        std::string instanceName(nameInstance);
        size_t pos = instanceName.find_last_of("/\\");
        if (pos != std::string::npos)
            instanceName = instanceName.substr(pos + 1);

        // gera um arquivo "bruto" no formato instancia, algoritmo, replicacao e resultado
        rawFile << instanceName << " "
                << algorithms[0] << " "
                << (run + 1) << " "
                << bestSolutionRun.ofv << "\n";
